...
- `src/` - TypeScript source (entry: `src/index.ts`)
  - `cache.ts` - cache store + MCP cached resource/download handling
  - `config.ts` - environment parsing, constants, types
  - `crypto.ts` - hashing and timing-safe comparisons
  - `errors.ts` - custom error classes
  - `fetch.ts` - URL validation/SSRF, fetching, telemetry
  - `http.ts` - Express server, auth, sessions, rate limiting
  - `mcp.ts` - MCP stdio server wiring
  - `observability.ts` - logging + request context
  - `tools.ts` - MCP tool handlers and schemas (`fetch-url`)
  - `transform.ts` - HTML-to-Markdown conversion + worker pool
...
4. Internal: project modules (e.g., `config`, `errors`, `fetch`, `transform`, `tools`, `http`, `mcp`)
5. Relative (`./`, `../`)
