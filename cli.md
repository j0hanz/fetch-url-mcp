# line API | Node.js v24.13.0 Documentation
...
Execute without arguments to start the [REPL](https://nodejs.org/docs/latest-v24.x/api/repl.html).

For more info about `node inspect`, see the [debugger](https://nodejs.org/docs/latest-v24.x/api/debugger.html) documentation.
...
- The file does not have a `.cjs` extension, and the nearest parent `package.json` file contains a top-level ["type"](https://nodejs.org/docs/latest-v24.x/api/packages.html#type) field with a value of `"module"`.

See [module resolution and loading](https://nodejs.org/docs/latest-v24.x/api/packages.html#module-resolution-and-loading) for more details.
...
If an option that takes a single value (such as `--max-http-header-size`) is passed more than once, then the last passed value is used. Options from the command line take precedence over options passed through the [NODE_OPTIONS](#node_optionsoptions) environment variable.
...
If this flag is passed, the behavior can still be set to not abort through [process.setUncaughtExceptionCaptureCallback()](https://nodejs.org/docs/latest-v24.x/api/process.html#processsetuncaughtexceptioncapturecallbackfn) (and through usage of the `node:domain` module that uses it).

#### `--allow-addons`[#](#--allow-addons)

Added in: v21.6.0, v20.12.0

When using the [Permission Model](https://nodejs.org/docs/latest-v24.x/api/permissions.html#permission-model), the process will not be able to use native addons by default. Attempts to do so will throw an `ERR_DLOPEN_DISABLED` unless the user explicitly passes the `--allow-addons` flag when starting Node.js.
...
This flag configures file system read permissions using the [Permission Model](https://nodejs.org/docs/latest-v24.x/api/permissions.html#permission-model).
...
Examples can be found in the [File System Permissions](https://nodejs.org/docs/latest-v24.x/api/permissions.html#file-system-permissions) documentation.
...
This flag configures file system write permissions using the [Permission Model](https://nodejs.org/docs/latest-v24.x/api/permissions.html#permission-model).
...
When using the [Permission Model](https://nodejs.org/docs/latest-v24.x/api/permissions.html#permission-model), the process will not be able to connect through inspector protocol.
...
The [v8.startupSnapshot API](https://nodejs.org/docs/latest-v24.x/api/v8.html#startup-snapshot-api) can be used to specify an entry point at snapshot building time, thus avoiding the need of an additional entry script at deserialization time:
...
For more information, check out the [v8.startupSnapshot API](https://nodejs.org/docs/latest-v24.x/api/v8.html#startup-snapshot-api) documentation.
...
- `builder` [<string>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#string_type) Required. Provides the name to the script that is executed before building the snapshot, as if [\--build-snapshot](#--build-snapshot) had been passed with `builder` as the main script name.
...
Provide custom [conditional exports](https://nodejs.org/docs/latest-v24.x/api/packages.html#conditional-exports) resolution conditions.
...
The default value is controlled by the [\--diagnostic-dir](#--diagnostic-dirdirectory) command-line option.
...
Warnings emitted from [process.emitWarning()](https://nodejs.org/docs/latest-v24.x/api/process.html#processemitwarningwarning-options) may contain a `code` and a `type`. This option will not-emit warnings that have a matching `code` or `type`.

List of [deprecation warnings](https://nodejs.org/docs/latest-v24.x/api/deprecations.html#list-of-deprecated-apis).

The Node.js core warning types are: `DeprecationWarning` and `ExperimentalWarning`

For example, the following script will not emit [DEP0025 require('node:sys')](https://nodejs.org/docs/latest-v24.x/api/deprecations.html#dep0025-requirenodesys) when executed with `node --disable-warning=DEP0025`:

```python
import sys from 'node:sys';const sys = require('node:sys');
```

For example, the following script will emit the [DEP0025 require('node:sys')](https://nodejs.org/docs/latest-v24.x/api/deprecations.html#dep0025-requirenodesys), but not any Experimental Warnings (such as [ExperimentalWarning: vm.measureMemory is an experimental feature](https://nodejs.org/docs/latest-v24.x/api/vm.html#vmmeasurememoryoptions) in <=v21) when executed with `node --disable-warning=ExperimentalWarning`:
...
Set the default value of `order` in [dns.lookup()](https://nodejs.org/docs/latest-v24.x/api/dns.html#dnslookuphostname-options-callback) and [dnsPromises.lookup()](https://nodejs.org/docs/latest-v24.x/api/dns.html#dnspromiseslookuphostname-options). The value could be:

- `ipv4first`: sets default `order` to `ipv4first`.
- `ipv6first`: sets default `order` to `ipv6first`.
- `verbatim`: sets default `order` to `verbatim`.

The default is `verbatim` and [dns.setDefaultResultOrder()](https://nodejs.org/docs/latest-v24.x/api/dns.html#dnssetdefaultresultorderorder) have higher priority than `--dns-result-order`.
...
Follows [ECMAScript module](https://nodejs.org/docs/latest-v24.x/api/esm.html#modules-ecmascript-modules) resolution rules.

Any query parameter or hash in the URL will be accessible via [import.meta.url](https://nodejs.org/docs/latest-v24.x/api/esm.html#importmetaurl).
...
The `--env-file-if-exists` flag is no longer experimental.
...
The `--env-file` flag is no longer experimental.
...
Loads environment variables from a file relative to the current directory, making them available to applications on `process.env`. The [environment variables which configure Node.js](#environment-variables_1), such as `NODE_OPTIONS`, are parsed and applied. If the same variable is defined in the environment and in the file, the value from the environment takes precedence.
...
> This flag is discouraged and may be removed in a future version of Node.js. Please use [\--import with register()](https://nodejs.org/docs/latest-v24.x/api/module.html#enabling) instead.

Specify the `module` containing exported [module customization hooks](https://nodejs.org/docs/latest-v24.x/api/module.html#customization-hooks). `module` may be any string accepted as an [import specifier](https://nodejs.org/docs/latest-v24.x/api/esm.html#import-specifiers).
...
#### `--experimental-require-module`[#](#--experimental-require-module)
...
See [Loading ECMAScript modules using require()](https://nodejs.org/docs/latest-v24.x/api/modules.html#loading-ecmascript-modules-using-require).
...
When used in conjunction with the `node:test` module, a code coverage report is generated as part of the test runner output. If no tests are run, a coverage report is not generated. See the documentation on [collecting code coverage from tests](https://nodejs.org/docs/latest-v24.x/api/test.html#collecting-code-coverage) for more details.
...
Enable [Web Storage](https://developer.mozilla.org/en-US/docs/Web/API/Web_Storage_API) support.
...
[Stability: 1](https://nodejs.org/docs/latest-v24.x/api/documentation.html#stability-index) \- Experimental. This flag is inherited from V8 and is subject to change upstream.
...
Disable loading native addons that are not [context-aware](https://nodejs.org/docs/latest-v24.x/api/addons.html#context-aware-addons).
...
Follows [ECMAScript module](https://nodejs.org/docs/latest-v24.x/api/esm.html#modules-ecmascript-modules) resolution rules. Use [\--require](#-r---require-module) to load a [CommonJS module](https://nodejs.org/docs/latest-v24.x/api/modules.html). Modules preloaded with `--require` will run before modules preloaded with `--import`.
...
4. If step 3 fails with an error code [ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX](https://nodejs.org/docs/latest-v24.x/api/errors.html#err_unsupported_typescript_syntax) or [ERR_INVALID_TYPESCRIPT_SYNTAX](https://nodejs.org/docs/latest-v24.x/api/errors.html#err_invalid_typescript_syntax), throw the error from step 2, including the TypeScript error in the message, else run as CommonJS.
...
The REPL does not support this option. Usage of `--input-type=module` with [\--print](#-p---print-script) will throw an error, as `--print` does not support ES module syntax.
...
See [V8 Inspector integration for Node.js](https://nodejs.org/docs/latest-v24.x/api/debugger.html#v8-inspector-integration-for-nodejs) for further explanation on Node.js debugger.

#### `--inspect-port=[host:]port`[#](#--inspect-porthostport)

Added in: v7.6.0

Set the `host:port` to be used when the inspector is activated. Useful when activating the inspector by sending the `SIGUSR1` signal. Except when [\--disable-sigusr1](#--disable-sigusr1) is passed.
...
V8 inspector integration allows tools such as Chrome DevTools and IDEs to debug and profile Node.js instances. The tools attach to Node.js instances via a tcp port and communicate using the [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/). See [V8 Inspector integration for Node.js](https://nodejs.org/docs/latest-v24.x/api/debugger.html#v8-inspector-integration-for-nodejs) for further explanation on Node.js debugger.
...
Disable using [syntax detection](https://nodejs.org/docs/latest-v24.x/api/packages.html#syntax-detection) to determine module type.

#### `--no-experimental-global-navigator`[#](#--no-experimental-global-navigator)

Added in: v21.2.0

Disable exposition of [Navigator API](https://nodejs.org/docs/latest-v24.x/api/globals.html#navigator) on the global scope.
...
Disable the experimental [node:sqlite](https://nodejs.org/docs/latest-v24.x/api/sqlite.html) module.
...
Disable type-stripping for TypeScript files. For more information, see the [TypeScript type-stripping](https://nodejs.org/docs/latest-v24.x/api/typescript.html#type-stripping) documentation.
...
The `file` name may be an absolute path. If it is not, the default directory it will be written to is controlled by the [\--diagnostic-dir](#--diagnostic-dirdirectory) command-line option.
...
Starts the Node.js command line test runner. This flag cannot be combined with `--watch-path`, `--check`, `--eval`, `--interactive`, or the inspector. See the documentation on [running tests from the command line](https://nodejs.org/docs/latest-v24.x/api/test.html#running-tests-from-the-command-line) for more details.
...
See the documentation on [global setup and teardown](https://nodejs.org/docs/latest-v24.x/api/test.html#global-setup-and-teardown) for more details.
...
A regular expression that configures the test runner to only execute tests whose name matches the provided pattern. See the documentation on [filtering tests by name](https://nodejs.org/docs/latest-v24.x/api/test.html#filtering-tests-by-name) for more details.
...
A test reporter to use when running tests. See the documentation on [test reporters](https://nodejs.org/docs/latest-v24.x/api/test.html#test-reporters) for more details.
...
A regular expression that configures the test runner to skip tests whose name matches the provided pattern. See the documentation on [filtering tests by name](https://nodejs.org/docs/latest-v24.x/api/test.html#filtering-tests-by-name) for more details.
...
Regenerates the snapshot files used by the test runner for [snapshot testing](https://nodejs.org/docs/latest-v24.x/api/test.html#snapshot-testing).
...
Set [tls.DEFAULT_MAX_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_max_version) to 'TLSv1.2'. Use to disable support for TLSv1.3.

#### `--tls-max-v1.3`[#](#--tls-max-v13)

Added in: v12.0.0

Set default [tls.DEFAULT_MAX_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_max_version) to 'TLSv1.3'. Use to enable support for TLSv1.3.

#### `--tls-min-v1.0`[#](#--tls-min-v10)

Added in: v12.0.0, v10.20.0

Set default [tls.DEFAULT_MIN_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_min_version) to 'TLSv1'. Use for compatibility with old TLS clients or servers.

#### `--tls-min-v1.1`[#](#--tls-min-v11)

Added in: v12.0.0, v10.20.0

Set default [tls.DEFAULT_MIN_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_min_version) to 'TLSv1.1'. Use for compatibility with old TLS clients or servers.

#### `--tls-min-v1.2`[#](#--tls-min-v12)

Added in: v12.2.0, v10.20.0

Set default [tls.DEFAULT_MIN_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_min_version) to 'TLSv1.2'. This is the default for 12.x and later, but the option is supported for compatibility with older Node.js versions.

#### `--tls-min-v1.3`[#](#--tls-min-v13)

Added in: v12.0.0

Set default [tls.DEFAULT_MIN_VERSION](https://nodejs.org/docs/latest-v24.x/api/tls.html#tlsdefault_min_version) to 'TLSv1.3'. Use to disable support for TLSv1.2, which is not as secure as TLSv1.3.
...
Prints information about usage of [Loading ECMAScript modules using require()](https://nodejs.org/docs/latest-v24.x/api/modules.html#loading-ecmascript-modules-using-require).
...
- `throw`: Emit [unhandledRejection](https://nodejs.org/docs/latest-v24.x/api/process.html#event-unhandledrejection). If this hook is not set, raise the unhandled rejection as an uncaught exception. This is the default.
- `strict`: Raise the unhandled rejection as an uncaught exception. If the exception is handled, [unhandledRejection](https://nodejs.org/docs/latest-v24.x/api/process.html#event-unhandledrejection) is emitted.
- `warn`: Always trigger a warning, no matter if the [unhandledRejection](https://nodejs.org/docs/latest-v24.x/api/process.html#event-unhandledrejection)hook is set or not but do not print the deprecation warning.
- `warn-with-error-code`: Emit [unhandledRejection](https://nodejs.org/docs/latest-v24.x/api/process.html#event-unhandledrejection). If this hook is not set, trigger a warning, and set the process exit code to 1.
...
This is equivalent to setting the [NODE_USE_ENV_PROXY=1](#node_use_env_proxy1) environment variable. When both are set, `--use-env-proxy` takes precedence.
...
On Windows and macOS, the certificate trust policy is similar to [Chromium's policy for locally trusted certificates](https://chromium.googlesource.com/chromium/src/+/main/net/data/ssl/chrome_root_store/faq.md#does-the-chrome-certificate-verifier-consider-local-trust-decisions), but with some differences:
...
Automatically zero-fills all newly allocated [Buffer](https://nodejs.org/docs/latest-v24.x/api/buffer.html#class-buffer) and [SlowBuffer](https://nodejs.org/docs/latest-v24.x/api/buffer.html#class-slowbuffer)instances.
...
When set to 1, the [module compile cache](https://nodejs.org/docs/latest-v24.x/api/module.html#module-compile-cache) can be reused across different directory locations as long as the module layout relative to the cache directory remains the same.
...
When set, the well known "root" CAs (like VeriSign) will be extended with the extra certificates in `file`. The file should consist of one or more trusted certificates in PEM format. A message will be emitted (once) with [process.emitWarning()](https://nodejs.org/docs/latest-v24.x/api/process.html#processemitwarningwarning-options) if the file is missing or malformed, but any errors are otherwise ignored.
...
This can also be enabled using the [\--use-env-proxy](#--use-env-proxy) command-line flag. When both are set, `--use-env-proxy` takes precedence.
...
This can also be enabled using the [\--use-system-ca](#--use-system-ca) command-line flag. When both are set, `--use-system-ca` takes precedence.

#### `NODE_V8_COVERAGE=dir`[#](#node_v8_coveragedir)

When set, Node.js will begin outputting [V8 JavaScript code coverage](https://v8project.blogspot.com/2017/12/javascript-code-coverage.html) and [Source Map](https://tc39.es/ecma426/) data to the directory provided as an argument (coverage information is written as JSON to files with a `coverage` prefix).
...
If the [\--openssl-config](#--openssl-configfile) command-line option is used, the environment variable is ignored.
...
While Node.js does not support all of the various [ways that TZ is handled in other environments](https://www.gnu.org/software/libc/manual/html_node/TZ-Variable.html), it does support basic [timezone IDs](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) (such as `'Etc/UTC'`, `'Europe/Paris'`, or `'America/New_York'`). It may support a few other abbreviations or aliases, but these are strongly discouraged and not guaranteed.
...
#### `--abort-on-uncaught-exception`[#](#--abort-on-uncaught-exception_1)

#### `--disallow-code-generation-from-strings`[#](#--disallow-code-generation-from-strings_1)

#### `--enable-etw-stack-walking`[#](#--enable-etw-stack-walking)

#### `--expose-gc`[#](#--expose-gc_1)
...
Sets the maximum [semi-space](https://www.memorymanagement.org/glossary/s.html#semi.space) size for V8's [scavenge garbage collector](https://v8.dev/blog/orinoco-parallel-scavenger) in MiB (mebibytes). Increasing the max size of a semi-space may improve throughput for Node.js at the cost of more memory consumption.

Since the young generation size of the V8 heap is three times (see [YoungGenerationSizeFromSemiSpaceSize](https://chromium.googlesource.com/v8/v8.git/+/refs/tags/10.3.129/src/heap/heap.cc#328) in V8) the size of the semi-space, an increase of 1 MiB to semi-space applies to each of the three individual semi-spaces and causes the heap size to increase by 3 MiB. The throughput improvement depends on your workload (see [#42511](https://github.com/nodejs/node/issues/42511)).
...
_line API | Node.js v24.13.0 Documentation_ | _#_ | [_Original Source_](https://nodejs.org/docs/latest-v24.x/api/cli.html) | _2026-02-06_
