...
import { logDebug, Loggers, logInfo, logWarn } from '../lib/core.js';
import {
  createAbortError,
  FetchError,
  getErrorMessage,
...
interface TransformExecutionOptions {
...
class WorkerPool implements TransformWorkerPool {
  ...
  constructor(
    ...
    this.capacity =
      size === 0
        ? 0
        : Math.max(POOL_MIN_WORKERS, Math.min(size, POOL_MAX_WORKERS));
    this.queueMax = POOL_MAX_WORKERS * QUEUE_CAPACITY_MULTIPLIER;
  }
  ...
  private createPendingTask(
    ...
    if (typeof htmlOrBuffer === 'string') {
      ...
      if (options.encoding) task.encoding = options.encoding;
    }
  ...
  private maybeScaleUp(): void {
    if (
      ...
      this.capacity < POOL_MAX_WORKERS
    ) {
...
let workerPathRef: string | null = null;

function getOrCreateWorkerPool(workerPath: URL): WorkerPool {
  const size = config.transform.maxWorkerScale === 0 ? 0 : POOL_MIN_WORKERS;
  const normalizedPath = workerPath.href;

  if (!workerPool) {
    workerPool = new WorkerPool(size, DEFAULT_TIMEOUT_MS, workerPath);
    workerPathRef = normalizedPath;
    logInfo(
  ...
  if (workerPathRef !== normalizedPath) {
    logWarn(
      'Ignoring transform worker pool path change after initialization',
      {
        previousWorkerPath: workerPathRef,
        nextWorkerPath: normalizedPath,
      },
      Loggers.LOG_TRANSFORM
    );
  }
...
export async function transformWithWorkerPool(
  htmlOrBuffer: string | Uint8Array,
  url: string,
  options: TransformExecutionOptions,
  runtime: WorkerRuntimeOptions
): Promise<MarkdownTransformResult> {
  const pool = getOrCreateWorkerPool(runtime.workerPath);
  if (pool.getCapacity() === 0) {
    throw new FetchError('Transform worker pool disabled', url, 0, {
      reason: 'worker_pool_disabled',
    });
  }

  if (typeof htmlOrBuffer === 'string') {
    return pool.transform(htmlOrBuffer, url, options);
  }
